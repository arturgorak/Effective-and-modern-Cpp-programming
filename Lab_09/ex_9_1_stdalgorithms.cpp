...
void toLowerAlpha(std::string & s1) {
    ...
    // erase - Usuwa jeden element lub wiele elementów z kontenera listy występujących na podanej pozycji lub w podanym zakresie
    // remove_if - Removes all elements satisfying specific criteria from the range [first, last) and returns a past-the-end iterator for the new end of the range.
...
int main(){
    ...
    // back_inserter - constructs a std::back_insert_iterator for the container c with the type deduced from the type of the argument.
    // <<>> (1 line)
    print(odd);

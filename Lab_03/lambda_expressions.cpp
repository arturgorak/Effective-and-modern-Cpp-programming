...
int main(){
  ...
  std::sort(v.begin(), v.end(), [center](int x, int y){
      return std::abs(center - x) - std::abs(center - y) < 0;
  });            // 1
  print(v, "center 50:");
  ...
  int start = 5, step =2;
  auto arithmeticGenerator =  [start, &step, i=int(0)]()mutable {
      int result = start + step* (i++);
      return result;
  } ;                   // [ 4 ]
  ...
  auto l1_norm = [](auto container) -> double {
      auto abs_adding = [](double x, double y){
          return std::abs(x) + std::abs(y);
      };
      return std::accumulate(container.begin(), container.end(), 0.0, abs_adding);
  };                         // [ 5 ]
